...
- Every session's media path state drops to `lost` ([ADR-0042](./0042-the-media-path-has-its-own-ladder.md)) and no session ends. A dead worker is exactly the failure that ladder exists to render: consoles that work perfectly, held by operators nobody can hear.
...
**No in-process restart is attempted.** A new Worker means a new Router, and every transport, producer and consumer belonged to the old one — so recovering in place is rebuilding every session's media path from a process that has just proved it can lose them. Letting the unit go down and come back is the same outcome, arrived at by a path that is exercised on every deployment.
