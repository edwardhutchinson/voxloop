...
| **Input** | A source reports intent, liveness, and whether its reading is stale-high ([ADR-0021](../adr/0021-ptt-input-is-a-level-with-liveness.md)). Sources are additive and a source never knows which emission mode it serves | — | **One source, and a lint rule.** The on-screen key control, whose liveness is whether its control is on screen; a source that goes away drops what it was holding rather than resuming it, which is v1 §7's stale-high rule arrived at from the one case that can produce it today. The keyboard bindings (#42) and the Tauri native hotkey (#61) are the variation this seam exists for, so until they land it is a boundary with a failing build behind it rather than a proven one |
