...
| Module | Interface promises | Calls | Exercised |
|---|---|---|---|
| **Session** | The socket, sign-in, assume, resume by session id, gap events ([ADR-0041](../adr/0041-a-session-is-resumed-by-name.md)). Owns the presence document and hands it out; nothing else talks to the server. **Connection state is the one thing it measures rather than receives** ([ADR-0018](../adr/0018-no-signalling-channel-means-no-emission-path.md)): it answers the server's heartbeats and counts the ones it is not getting, on the ladder the heartbeat itself carries | — | Real |
| **Input** | A source reports two things and no third — its intent, and its liveness ([ADR-0021](../adr/0021-ptt-input-is-a-level-with-liveness.md)). Stale-high is a rule each source holds rather than a field it publishes: a reading that was high across a withdrawal is dropped rather than resumed, so it reaches the OR as an ordinary low (v1 §7). Sources are additive and a source never knows which emission mode it serves | — | **Two sources, a binding per mode, and a lint rule.** The on-screen key control, whose liveness is whether its control is on screen, and the keyboard, whose liveness is whether there is a window to listen on and a role to key under. Neither can name a mode: the seam is handed a binding per name from above and reports under the same names, and the modes are read off it in `modes.js`. A source that goes away drops what it was holding rather than resuming it, and autorepeat may not raise a level that is low — v1 §7's stale-high rule, arrived at from the two cases that produce it. The Tauri native hotkey (#61) is the remaining variation, and it is a third file under `sources/` |
...
| **Console** | Board and ledger over one loop list, the transmit bar, the hail picker ([ADR-0032](../adr/0032-the-console-is-two-views-of-one-loop-list.md)). Renders server-pushed state and never optimistically. **The emission modes are its** — momentary and latched, above the seam and never inside a source ([ADR-0022](../adr/0022-latch-is-never-derived-from-a-momentary-press.md)) | Session, Input, Audio | Real |
